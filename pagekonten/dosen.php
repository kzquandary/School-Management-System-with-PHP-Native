                                    ...
                                    <table id="myTable" class="display table table-bordered table-striped" style="min-width: 100px;">
                                        <thead>
                                            <tr>
                                                ...
                                                <th onclick="sortTable(3)"><a href="javascript:void()">No Hp</a></th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            <?php
                                            $dosen = "SELECT * FROM dosen";
                                            $qdosen = mysqli_query($con, $dosen);
                                            foreach ($qdosen as $data){
                                            ?>
                                            <tr>
                                                <td><img class="rounded-circle" width="50" src="<?= $data['profil']; ?>" alt=""></td>
                                                <td><a href="profil.php?id=<?= $data['kode_dosen']; ?>"><font color="blue"><?= $data['nama']; ?></a></font></td>
                                                <td><?= $data['email']; ?></td>
                                                <td><?= $data['gender']; ?></td>
                                                <td><?= $data['no_hp']; ?></td>											
                                            </tr>
